...
async def process_tool_calls(tool_calls, stream_writer=None):
    tool_messages = [
        AIMessage(
            content='',
        ...
        if tool:
            try:
                ...
                if tool_call_description:
                    stream_writer.write_event(tool_call_description, model)
                
                tool_result = await tool.coroutine(args, stream_writer=stream_writer)
...
async def stream_completion(completion_id, messages, include_tool_messages, max_tokens, temperature, stream_writer=None):
    async for event in _stream_completion(completion_id, messages, include_tool_messages, max_tokens, temperature, stream_writer):
        yield event

async def _stream_completion(completion_id, messages, include_tool_messages, max_tokens, temperature, stream_writer=None):
    llm = ChatOpenAI(
        api_key=api_key,
        model=model,
        max_tokens=max_tokens,
            ...
            async for event in events:
                ...
                if event['event'] == 'on_chat_model_stream':
                    ...
                    if event['data']['output'].additional_kwargs.get('tool_calls'):
                        ...
                        tool_messages = await process_tool_calls(tool_calls, stream_writer)
                        conversation.extend(tool_messages)
                        has_tool_calls = True
