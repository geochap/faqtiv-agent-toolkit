...
import { extractFunctionNames, extractFunctionCode } from '../lib/parse-utils.js';
...
function codeResponse(response) {
  try {
    ...
    const rawCode = stripCodeFences(response);
    const adhocTaskCode = extractFunctionCode(rawCode, 'doAdhocTask');
    const taskCode = extractFunctionCode(rawCode, 'doTask');
    if (!taskCode) {
      throw new Error(`Failed to parse function code: ${rawCode}`);
    }

    const code = adhocTaskCode ? `${adhocTaskCode}\n\n${taskCode}` : taskCode;
    const call = 'doAdhocTask()';

    if (!code) {
      console.warn('Could not generate an answer from AI response');
      throw new Error('AI response: ' + rawCode);
    }
