...
async function doTask(agentDirectoryPath, taskName, argsPacked) {
  const args = argsPacked.split(',').map(arg => `"${arg.replace(/"/g, '\\"')}"`).join(' ');
  const result = await executeAgentCommand(agentDirectoryPath, `run-task ${taskName} ${args}`);
