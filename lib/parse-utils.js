...
// UTILITY FUNCTIONS TO PARSE IMPORT STATEMENTS

function normalizeJSImport(importStatement) {
  // Check if it's an ES6 import
  const es6Match = importStatement.match(/import(.*?)from['"](.+?)['"]/);
  if (es6Match) {
  ...
  const commonJSMatch = importStatement.match(/(?:const|let|var)(.*?)=require\(['"](.+?)['"]\)/);
  if (commonJSMatch) {
...
function normalizePythonImport(importStatement) {
  // Check if it's a Python "import x" statement
  const simpleImportMatch = importStatement.match(/import([A-Za-z0-9_]+)/);
  if (simpleImportMatch) {
    const [, moduleName] = simpleImportMatch;
    return { moduleName, importPart: moduleName };
  }

  // Check if it's a Python "from x import y" statement
  const fromImportMatch = importStatement.match(/from([A-Za-z0-9_.]+)import(.+)/);
  if (fromImportMatch) {
    const [, moduleName, importPart] = fromImportMatch;
    return { moduleName, importPart };
  }
}

export function getNormalizedImport(importStatement, runtime) {
  // Remove all whitespace
  const strippedImport = importStatement.replace(/\s+/g, '');
  const parsers = {
    javascript: normalizeJSImport,
    python: normalizePythonImport
  };

  return parsers[runtime || getRuntime()](strippedImport);
}

// UTILITY FUNCTIONS TO REMOVE OUTER CODE BLOCKS FROM AI RESPONSES

export function removeOuterCodeBlock(text) {
